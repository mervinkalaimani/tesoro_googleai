...
/**
 * Case releases have no seed either, for a different reason: "2026 K Case" is
 * this month's box, and it only exists once a car out of it is in the table. Still
 * free text — the list is there because it repeats exactly and was being retyped.
 */
export const CASE_SEED: string[] = [];
 ...
 */
function rank(collected: (string | undefined)[], seed: string[]): string[] {
...
/**
 * Assortments for one brand — "Premium" and "Silver Series" for Hotwheels,
 * "Blister" and "Qube Carz" for Mini GT.
 *
 * Narrowed the way models are narrowed by make: an assortment is a brand's own
 * range, and the flat list was offering Mini GT's alongside Matchbox's. There is
 * no generated by-brand seed to narrow — assortments are one short vocabulary,
 * not a per-make tree — so the collection does the narrowing on its own.
 *
 * Only that brand's, with no fallback: a brand nobody has bought yet offers
 * nothing, which says "type it" more clearly than a list of another maker's
 * ranges. The combobox still accepts anything, and what is typed becomes an
...
/**
 * Model years already in the collection, newest first.
 *
 * Ranked by recency rather than by count, unlike every other list here: a year is
 * read as a number, and "1969 before 1995 because there are more of them" is a
 * list nobody can scan. Values that are not four digits — "'71", "c.1970" — keep
 * their place at the end rather than being dropped, since they are what somebody
 * actually typed.
 */
export function yearOptionsFor(cars: Diecast[]): string[] {
  const seen = new Set<string>();
  for (const c of cars) {
    const y = String(c.year ?? "").trim();
    if (y) seen.add(y);
  }
  const four = (v: string) => (/^\d{4}$/.test(v) ? Number(v) : null);
